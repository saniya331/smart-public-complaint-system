
const te = {
  // Navbar
  home: "హోమ్",
  login: "లాగిన్",
  register: "నమోదు",
  dashboard: "డాష్‌బోర్డ్",
  logout: "సైన్ అవుట్",
  notifications: "నోటిఫికేషన్లు",

  // Home
  tagline: "స్మార్ట్ పబ్లిక్ గ్రీవెన్స్ పోర్టల్",
  heroTitle1: "మీ నగరం. మీ స్వరం.",
  heroTitle2: "స్పష్టమైన చర్య.",
  heroDescription:
    "స్థానిక సమస్యలను నివేదించండి, ప్రతి అప్‌డేట్‌ను ట్రాక్ చేయండి మరియు మరింత స్పందించే సమాజాన్ని నిర్మించడంలో సహాయపడండి.",
  raiseComplaint: "ఫిర్యాదు చేయండి",
  trackComplaint: "ఫిర్యాదును ట్రాక్ చేయండి",
  liveCivicResponse: "లైవ్ సివిక్ రెస్పాన్స్",
  issuesResolved: "ఈ నెల పరిష్కరించిన సమస్యలు",
  improvement: "↑ 18% మెరుగుదల",

  // Citizen Dashboard
  citizenPortal: "పౌర పోర్టల్",
  goodMorning: "శుభోదయం",
  dashboardDescription:
    "మీ పౌర ఫిర్యాదుల సమీక్ష ఇక్కడ ఉంది.",
  newComplaint: "కొత్త ఫిర్యాదు",
  totalComplaints: "మొత్తం ఫిర్యాదులు",
  inProgress: "పురోగతిలో ఉంది",
  resolved: "పరిష్కరించబడింది",
  assigned: "కేటాయించబడింది",
  recentComplaints: "ఇటీవలి ఫిర్యాదులు",
  recentComplaintsDescription:
    "మీరు సమర్పించిన సమస్యల తాజా పురోగతిని ట్రాక్ చేయండి.",
  viewAll: "అన్నీ చూడండి",

  // My Complaints
  myComplaints: "నా ఫిర్యాదులు",
  myComplaintsDescription:
    "మీరు సమర్పించిన అన్ని ఫిర్యాదులను చూడండి మరియు నిర్వహించండి.",
  searchComplaints: "ఫిర్యాదులను వెతకండి...",
  allStatuses: "అన్ని స్థితులు",
  submitted: "సమర్పించబడింది",
  showing: "చూపిస్తున్నవి",
  of: "లో",
  complaints: "ఫిర్యాదులు",
  noComplaintsFound: "ఫిర్యాదులు కనుగొనబడలేదు",
  changeSearch:
    "మీ శోధన లేదా స్థితి ఫిల్టర్‌ను మార్చి ప్రయత్నించండి.",

  // Complaint Details
  complaintDetails: "ఫిర్యాదు వివరాలు",
  complaintInformation: "ఫిర్యాదు సమాచారం",
  category: "వర్గం",
  location: "స్థానం",
  submittedDate: "సమర్పించిన తేదీ",
  currentStatus: "ప్రస్తుత స్థితి",
  complaintTracking: "ఫిర్యాదు ట్రాకింగ్",
  complaintSubmitted: "ఫిర్యాదు సమర్పించబడింది",
  complaintAssigned: "ఫిర్యాదు కేటాయించబడింది",
  workInProgress: "పని పురోగతిలో ఉంది",
  complaintResolved: "ఫిర్యాదు పరిష్కరించబడింది",
  backToComplaints: "నా ఫిర్యాదులకు తిరిగి వెళ్లండి",

  complaintNotFound: "ఫిర్యాదు కనుగొనబడలేదు",
complaintDoesNotExist:
  "మీరు వెతుకుతున్న ఫిర్యాదు అందుబాటులో లేదు.",
complaintId: "ఫిర్యాదు ID",
submittedMessage:
  "మీ ఫిర్యాదు విజయవంతంగా సమర్పించబడింది.",
assignedMessage:
  "ఫిర్యాదు బాధ్యతగల అధికారికి కేటాయించబడుతుంది.",
progressMessage:
  "బాధ్యతగల విభాగం ఫిర్యాదుపై పని చేస్తోంది.",
resolvedMessage:
  "ఫిర్యాదు పరిష్కరించబడింది.",

  // Submit Complaint
  raiseAComplaint: "ఫిర్యాదు చేయండి",
  submitDescription:
    "వివరాలను అందించండి. మేము దానిని సరైన విభాగానికి పంపిస్తాము.",
  whatIsTheIssue: "సమస్య ఏమిటి?",
  selectCategoryDescription:
    "వేగంగా కేటాయించడానికి సరైన వర్గాన్ని ఎంచుకోండి.",
  complaintCategory: "ఫిర్యాదు వర్గం",
  selectCategory: "వర్గాన్ని ఎంచుకోండి",
  issueTitle: "సమస్య శీర్షిక",
  describeIssue: "సమస్యను వివరించండి",
  descriptionPlaceholder:
    "సమస్యను స్పష్టంగా వివరించండి. సమస్య ఎంతకాలంగా ఉందో వంటి ఉపయోగకరమైన వివరాలను చేర్చండి.",
  whereIsTheIssue: "సమస్య ఎక్కడ ఉంది?",
  locationDescription:
    "విభాగం చర్య తీసుకోవడానికి స్థానాన్ని అందించండి.",
  district: "జిల్లా",
  selectDistrict: "జిల్లాను ఎంచుకోండి",
  mandalLocality: "మండలం / ప్రాంతం",
  selectLocality: "ప్రాంతాన్ని ఎంచుకోండి",
  exactLocation: "ఖచ్చితమైన స్థానం లేదా సమీప ల్యాండ్‌మార్క్",
  landmarkPlaceholder:
    "ఉదాహరణ: కమ్యూనిటీ లైబ్రరీ దగ్గర, 3వ క్రాస్ రోడ్",
  uploadEvidence: "ఫిర్యాదు ఆధారాన్ని అప్‌లోడ్ చేయండి",
  optional: "ఐచ్ఛికం",
  uploadImage: "＋ చిత్రాన్ని అప్‌లోడ్ చేయండి",
  imageFormat: "JPG లేదా PNG, గరిష్ట పరిమాణం 10 MB",
  cancel: "రద్దు చేయండి",
  submitComplaint: "ఫిర్యాదు సమర్పించండి",
  submitting: "సమర్పిస్తోంది...",

  // Categories
  electricity: "విద్యుత్",
  sanitation: "పారిశుధ్యం",
  roadsTransport: "రోడ్లు & రవాణా",
  waterSupply: "నీటి సరఫరా",
  publicSafety: "ప్రజా భద్రత",

  viewDetails: "వివరాలను చూడండి",

  // Login
welcomeBack: "తిరిగి స్వాగతం",
everyConcernResponse: "ప్రతి సమస్యకు స్పందన అవసరం.",
trackComplaintsBetterCity:
  "మీ ఫిర్యాదులను ట్రాక్ చేసి మెరుగైన నగరాన్ని నిర్మించడంలో సహాయపడండి.",
backToHome: "హోమ్‌కు తిరిగి వెళ్లండి",
signInContinue: "మీ డ్యాష్‌బోర్డ్‌ను కొనసాగించడానికి సైన్ ఇన్ చేయండి.",
emailAddress: "ఇమెయిల్ చిరునామా",
emailPlaceholder: "you@example.com",
password: "పాస్‌వర్డ్",
passwordPlaceholder: "మీ పాస్‌వర్డ్‌ను నమోదు చేయండి",
rememberMe: "నన్ను గుర్తుంచుకోండి",
forgotPassword: "పాస్‌వర్డ్ మర్చిపోయారా?",
signIn: "సైన్ ఇన్",
newToCivicVoice: "CivicVoice కి కొత్తగా వచ్చారా?",
createAccount: "ఖాతాను సృష్టించండి",

// Register
joinCommunity: "మీ సంఘంలో చేరండి",
makeVoiceCount: "మీ స్వరానికి విలువ ఇవ్వండి.",
raiseIssuesImproveServices:
  "ముఖ్యమైన సమస్యలను నివేదించి ప్రజా సేవలను మెరుగుపరచడంలో సహాయపడండి.",
registerReportTrack:
  "పౌర సమస్యలను నివేదించడానికి మరియు ట్రాక్ చేయడానికి నమోదు చేసుకోండి.",
fullName: "పూర్తి పేరు",
fullNamePlaceholder: "మీ పూర్తి పేరును నమోదు చేయండి",
mobileNumber: "మొబైల్ నంబర్",
createPasswordPlaceholder: "పాస్‌వర్డ్‌ను సృష్టించండి",
alreadyHaveAccount: "ఇప్పటికే ఖాతా ఉందా?",
};

export default te;